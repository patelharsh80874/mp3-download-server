...
app.get("/generate-audio", async (req, res) => {
    ...
    try {
        // Download the audio and image files into memory
        const audioStream = await downloadFile(audioUrl);
        const imageStream = await downloadFile(imageUrl);

        // Convert audio to MP3 in memory
        const convertedAudioStream = await convertAudioToMp3(audioStream);

        // Convert image to JPG in memory
        const convertedImageStream = await convertImageToJpg(imageStream);

        // Process the MP3 file and embed metadata (song name, year, album)
        ffmpeg(convertedAudioStream)
            .input(convertedImageStream)
            .outputOptions("-map", "0", "-map", "1", "-c", "copy")
            .outputOptions("-metadata", `title=${songName}`)
            .outputOptions("-metadata", `year=${year}`)
            .outputOptions("-metadata", `album=${album}`)
            ...
            .pipe(res, { end: true }); // Send the processed file as a response to the client
    } catch (err) {
        console.error(err);
        res.status(500).send({ error: "Something went wrong!" });
    }
...
// Helper function to download a file into memory
...
// Helper function to convert audio to MP3 in memory (with 320kbps)
function convertAudioToMp3(inputStream) {
    return new Promise((resolve, reject) => {
        const outputStream = new stream.PassThrough();
        ffmpeg(inputStream)
            .toFormat("mp3")
            .audioBitrate(320)  // Set audio bitrate to 320kbps
            .on("end", () => resolve(outputStream))
            .on("error", reject)
            .pipe(outputStream, { end: true });
    });
}

// Helper function to convert image to JPG in memory
function convertImageToJpg(inputStream) {
    return new Promise((resolve, reject) => {
        const outputStream = new stream.PassThrough();
        ffmpeg(inputStream)
            .toFormat("mjpeg")
